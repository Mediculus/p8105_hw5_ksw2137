...
progresses while control stays relatively flat. This suggests treatment
in experimental group have a positive correlation while control has ~0
correlation (which is expected).
...
linear regression simulation, which has a general formula of: yi = β0 +
β1 xi1 + ϵi
...
Our resulting dataframe contains 70000 rows which correspond to 10^{4}
observations per true β1. It has 3 columns, which stores the true
`beta1`, `beta1_hat`, and `p_val`.
...
We could also plot the overall average estimates of β̂1 compared against
the true β1 and overlay the average estimate for β̂1 from samples with
rejected
null.
...
Per above, our average of β̂1 estimate of the rejected nulls are only
approximately equal at β1 = 0 and as it approaches β1 = 6. The reason is
likely because we are taking non-random sample of β̂ (filter by p-val \<
0.05), which biases our sample. Because of this, the “closeness” to the
true β1 depends on the power. This is because as our effect size
increase, we “capture” more non-random “samples” and eventually
obtaining the “whole sample” - resulting in a better estimate (less
biased). β̂1 at 0 is approximately equal to the β1 is a slightly unique
case as it behaves like the “opposite” of β1 = 6. Where β1 = 6 captures
“most” of the estimates, β1 = 0 captures “almost none”, thus also
closely approximating the true β1.
